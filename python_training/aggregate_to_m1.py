"""
M1 preparation with session filtering.
"""
import sys
import yaml
import pandas as pd
...
def main():
    print("=" * 70)
    print("M1 Preparation with Session Filtering")
    print("=" * 70)
    ...
    if not m1_parquet.exists():
        print(f"ERROR: {m1_parquet} not found. Run download_or_import_data.py first.")
        sys.exit(1)
    ...
    m1_csv = output_dir / 'xauusd_m1_clean.csv'
    df.to_csv(m1_csv, index=False)
    print(f"\nSaved full M1: {m1_csv.name}")
    
    # Save session-only
    df_session = df[df['session_flag'] == 1].copy()
    session_csv = output_dir / 'xauusd_m1_session.csv'
    df_session.to_csv(session_csv, index=False)
    print(f"Saved session M1: {session_csv.name} ({len(df_session):,} rows)")
